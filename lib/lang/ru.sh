...
T["no desktop"]="без рабочего стола"
...
T["No desktop was chosen: nothing to theme."]="Рабочий стол не выбран: применять тему не к чему."
T["Do you want Reimu to set a theme and icons? (No leaves the desktop as it comes)"]="Хотите, чтобы Reimu настроил тему и значки? (Нет — оставить рабочий стол как есть)"
T["Theme"]="Тема"
...
вашего пользователя, так что рабочий стол будет выглядеть так с первого входа.
Это темы GTK: в XFCE, GNOME, Cinnamon, MATE, Budgie и оконных менеджерах они
охватывают весь рабочий стол; в KDE Plasma, LXQt и COSMIC они применяются к
GTK-приложениям и значкам, а сам рабочий стол становится тёмным или светлым
в тон. Темы «AUR» требуют AUR-помощника из раздела «Программы».
Это необязательно: ответьте «Нет», и рабочий стол останется ровно таким, как есть.
EOF
}
