...
config_validate() {
  ...
  case "$REIMU_BOOTLOADER" in systemd-boot|grub) ;; *) problems+=("Bootloader must be systemd-boot or grub (got '$REIMU_BOOTLOADER').");; esac
  if [[ "$REIMU_BOOTLOADER" == systemd-boot && "$DETECT_FIRMWARE" == bios ]]; then
    problems+=("systemd-boot needs UEFI; this machine booted in BIOS mode. Use grub.")
  fi
